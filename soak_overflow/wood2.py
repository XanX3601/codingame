...
class TileMapMask:
    ...
    def __add__(self, other_mask):
        if not isinstance(other_mask, TileMapMask):
            raise TypeError()

        result = TileMapMask(self.tile_map)
        for xy, value in self.xy_to_value.items():
            result.xy_to_value[xy] = value
        for xy, value in other_mask.xy_to_value.items():
            if xy not in result.xy_to_value:
                result.xy_to_value[xy] = value
            else:
                result.xy_to_value[xy] += value
...
def get_damage_mask(tile_map, agent_collection, agent_id):
    ...
    for enemy_agent_id in enemy_agent_ids:
        ...
        tiles = [enemy_tile] + enemy_tile.neighbors
...
def main():
    ...
    while True:
        ...
        sum_damage_mask = TileMapMask(tile_map)
        for enemy_agent_id in enemy_agent_ids:
            damage_mask = get_damage_mask(tile_map, agent_collection, enemy_agent_id)
            sum_damage_mask = sum_damage_mask + damage_mask

        sum_damage_mask.debug()

        for my_agent_id in my_agent_ids:
            my_agent = agent_collection.get_agent(my_agent_id)
            agent_tile = tile_map.get_tile(my_agent.x, my_agent.y)

            tiles = [agent_tile] + agent_tile.neighbors

            tiles.sort(key=lambda tile: sum_damage_mask.get_value(tile.x, tile.y))
            move_tile = tiles[0]

            damage_mask = get_damage_mask(tile_map, agent_collection, my_agent_id)

            enemy_agents = [agent_collection.get_agent(enemy_agent_id) for enemy_agent_id in enemy_agent_ids]
            enemy_agents.sort(key=lambda agent: damage_mask.get_value(agent.x, agent.y), reverse=True)
            target_agent = enemy_agents[0]

            print(f"{my_agent.id}; MOVE {move_tile.x} {move_tile.y}; SHOOT {target_agent.id}")
